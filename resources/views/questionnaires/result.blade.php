        ...
        <div class="post-wrapper">
            <div class="post-body"><div class="post-content">
                    ...
                    <div class="row top-padding bottom-padding single-h-padding limit-width row-parent" data-imgready="true" style="padding-top:20px;">
                        ...
                        <div class="card-header"><h1 class="pull-left">{{ $questionnaire->title }}</h1></div>
                      ...
                      <div class="btnwr1" data-imgready="true" style="width:100%; margin:25px auto 20px; text-align:center">
                          ...
                          <a class="btn btn-dark" href="/takepoll/{{ $questionnaire->id }}-{{ str_slug($questionnaire->title)}}">Take Survey</a>
                      </div>
...
<div class="clearfix"></div>
                        <?php //dd($questionnaire->pollquestions);?>
                        
                          <div class="box-body">

                              @foreach($questionnaire->pollquestions as $question)
                                  <div class="card mt-4">

                                      <div class="flow-text">{{ $question->question }}</div>

                                      <div class="card-body">

<ul>
                                              @foreach($question->pollanswers as $answer)
                                              <li style="margin:0px; padding:0px; justify-content-lg-between;">
                                                  <div>
                                                      {{ $answer->answer }}
<span style="text-align: right">
                                                  @if($question->pollresponses->count())

                                                     ( {{ intval(($answer->pollresponses->count() * 100) / $question->pollresponses->count())}}%)
