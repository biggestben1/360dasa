...
<div class="row-inner" style=" margin-bottom: -1px;">
    ...
    <form action="{{ route('surveyaddtest') }}" method="post">
    ...
    foreach ($json_a as $person_name => $person_a) {
        ...
        $count = $count+1;
        $result = array_unique($person_a['objgroup']);
          ...
          <li> <strong style=" color: #00AA00;  font-size:25px; font-weight: lighter"><?php echo  $person_a['question'][$i];?></strong><br>
            ...
            <fieldset class="m" >
                ...
                <ol type="A">
                    ...
                    <input type="hidden" name="obj[]" value="<?php echo  $person_a['obj'][$i];?>">
                    <?php //echo $i;
                    //$result = array_unique($person_a['objgroup']);
                    //print_r($result);
                        //echo $yourUniquePids[$i];
                    $objgroup = $yourUniquePids[$i];?>
                   <?php for($a=0; $a<count($person_a['obj']); $a++) {
                        $objgroupaa= $person_a['objgroup'][$a];
                       //echo "<br>";
                   if($objgroupaa==$objgroup){
                    //$total = $total+1;

                        ?>
                        <li><input type="radio" id="inlineCheckbox2" value="<?php echo $a;?>" name="appsDetails[][q<?php echo $count;?>]"><i></i></label><?php echo $person_a['obj'][$a];?>  </li><br>
                    <?php
                    }
                    }?>
                </ol>
