...
function addField(fieldType) {
    ...
    if (hasChoices) {
        includeChoicesHTML = '' +
            '<div class="choices">' +
            '<ul></ul>' +
            '<button type="button" class="add-choice"><i class="fa fa-plus" aria-hidden="true"></i></button>' +
            '</div>'
    }
...
function addChoice() {
    return '' +
        ...
        '</label>' +
        '<button type="button" class="delete delete-choice"><i class="fa fa-minus" aria-hidden="true"></i></button>' +
        '</li>'
}

//Loads a saved form from your database into the builder
function loadForm(formID) {
    $.getJSON('sjfb-load.p?form_id=' + formID, function(data) {
        if (data) {
