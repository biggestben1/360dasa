    ...
    $.getJSON('sjfb-load.php?form_id=' + formID, function(data) {
        if (data) {
            ...
            $.each( data, function( k, v ) {
                ...
                if (v['req']) {
					...
					else if (fieldType == 'tel') { $currentField.find("input").prop('required',true).addClass('required-choice') }
					...
					else if (fieldType == 'checkbox') { $currentField.find("input").prop('required',true).addClass('required-choice') }
        ...
        function addFieldHTML(fieldType) {
            ...
            switch (fieldType) {
				...
				case 'tel':
                    return '' +
                        '<div id="sjfb-' + uniqueID + '" class="sjfb-field sjfb-tel">' +
                        '<label for="tel-' + uniqueID + '"></label>' +
                        '<input type="tel" id="tel-' + uniqueID + '">' +
                        '</div>';
				
				case 'email':
                    return '' +
                        '<div id="sjfb-' + uniqueID + '" class="sjfb-field sjfb-email">' +
                        '<label for="email-' + uniqueID + '"></label>' +
                        '<input type="email" id="email-' + uniqueID + '">' +
                        '</div>';
				...
				case 'date':
                    return '' +
                        ...
                        '<input type="date" id="date-' + uniqueID + '">' +
                        '</div>';
						
				case 'file':
                    return '' +
                        '<div id="sjfb-' + uniqueID + '" class="sjfb-field sjfb-file">' +
                        '<label for="file-' + uniqueID + '"></label>' +
                        '<input type="file" id="file-' + uniqueID + '">' +
                        '</div>';
